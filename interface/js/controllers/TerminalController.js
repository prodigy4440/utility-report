 ...
 */
angular.module('MetronicApp').controller('TerminalController',
    function ($rootScope, $scope, $http, $timeout, API, Toast, LocalStorage, $window) {
        ...
        $scope.loadTerminalUsers = function () {
            $scope.page = 0;
            $scope.size = 20;
            ...
            var result = API.getTerminalUsers($scope.config);

            result.success(function (data) {
                if (data.status.code === 0) {
                    console.log(data);
                    $scope.terminalusers = data.entity;
